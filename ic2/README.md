...
# Start a background server (returns once it is ready):
...
# Check a file — live progress bars, exit 0 on success, 1 on first error:
...
`$ISABELLE_HOME_USER/ic2/<name>.log` (override with `-L FILE`). Use
`ic2 server attach` to follow a backgrounded server's log — e.g. to watch a cold
heap build.
...
|---|---|
| 0 | all checks passed |
| 1 | a check failed (error found, or run stopped) |
| 2 | bad usage (e.g. no FILE) |
| 3 | server unreachable, or connection dropped mid-check |
...
Key flags beyond the `isabelle jedit` set: `--daemon` (background + return once
ready), `-n NAME` (server name, so several coexist), `-N` (no build — fail fast
if the heap is missing), `--no-iq` / `--mcp` (see below).
...
`McpServer` on loopback (port 8765, scanning upward to the first free port) and
registers three tool families:
  ...
  * **session diagnostic tools** — `list_files`, `get_diagnostics`,
    ...
    `get_command_info`, `get_state_at`, etc. These read only the document
    snapshot (base PIDE, no jEdit), so the identical code serves ic2's headless
    session and I/Q's live PIDE session — the `query` CLI routes through the same
    dispatch;
  * **`check` / `check_async` / `check_status` / `check_cancel`** — the MCP
    analogue of `isabelle ic2 check` (with MCP progress notifications).

The MCP token comes from `IQ_AUTH_TOKEN` (the same variable I/Q uses) if set,
else it is generated and reported by `ic2 server status`; clients call the
`authenticate` tool with it before any other. The in-prover `ML_Repl` is never
advertised, so nothing can connect around the bridge to the prover.

Bring-up is best-effort: if the `ir/` sources are missing, `python3`/`repl.py` is
unavailable, or the MCP layer fails, checking still works. repl.py runs with
...
the client derives the same path from the name. There is no auth token for the
control socket — access control is the *parent directory*, which `ic2 server
start` creates mode `0700` (owner-only), the same model as the ssh-agent / tmux
socket directories. (The I/R and MCP endpoints layered on top are separately
token-authenticated, as above.) A crashed server can leave a stale socket; on
startup the server probes it, reclaims it if nothing is listening, and refuses to
start if a live server already holds the name.
