...
definition overflowing_sub :: \<open>'l::{len} word \<Rightarrow> 'l word \<Rightarrow>
    ('machine, 'l word \<times> bool \<times> tnil, 'abort, 'i, 'o) function_body\<close> where
  \<open>overflowing_sub x y \<equiv> FunctionBody (literal (x - y, x < y, TNil))\<close>
...
definition abs_diff_core :: \<open>'l::{len} word \<Rightarrow> 'l word \<Rightarrow> 'l word\<close> where
  \<open>abs_diff_core x y \<equiv> if x < y then y - x else x - y\<close>

definition abs_diff :: \<open>'l::{len} word \<Rightarrow> 'l word \<Rightarrow> ('s, 'l word, 'abort, 'i, 'o) function_body\<close> where
  \<open>abs_diff x y \<equiv> FunctionBody (literal (abs_diff_core x y))\<close>
...
definition checked_sub_core :: \<open>'a::{len} word \<Rightarrow> 'a word \<Rightarrow> 'a word option\<close> where
  \<open>checked_sub_core w v \<equiv> if w < v then None else Some (w - v)\<close>

definition checked_div_core ::  \<open>'a::{len} word \<Rightarrow> 'a word \<Rightarrow> 'a word option\<close> where
  \<open>checked_div_core w v \<equiv> if v = 0 then None else Some (w div v)\<close>
...
definition checked_sub :: \<open>'a::{len} word \<Rightarrow> 'a word \<Rightarrow> ('s, 'a word option, 'abort, 'i, 'o) function_body\<close> where
  \<open>checked_sub w v \<equiv> FunctionBody (literal (checked_sub_core w v))\<close>

definition checked_div :: \<open>'a::{len} word \<Rightarrow> 'a word \<Rightarrow> ('s, 'a word option, 'abort, 'i, 'o) function_body\<close> where
  \<open>checked_div w v \<equiv> FunctionBody (literal (checked_div_core w v))\<close>
...
definition overflowing_sub_contract :: \<open>'l::{len} word \<Rightarrow> 'l word \<Rightarrow>
      ('machine::{sepalg}, 'l word \<times> bool \<times> tnil, 'b) function_contract\<close> where
  [crush_contracts]: \<open>overflowing_sub_contract x y \<equiv>
    let pre = \<langle>True\<rangle> in
    let post = \<lambda>ret. \<langle>ret = (x - y, x < y, TNil)\<rangle> in
      make_function_contract pre post\<close>
ucincl_auto overflowing_sub_contract

lemma overflowing_sub_spec [crush_specs]:
  shows \<open>\<Gamma> ; overflowing_sub x y \<Turnstile>\<^sub>F overflowing_sub_contract x y\<close>
by (crush_boot f: overflowing_sub_def contract: overflowing_sub_contract_def) crush_base
...
definition abs_diff_contract ::\<open>'l::{len} word \<Rightarrow> 'l word \<Rightarrow>
      ('machine::{sepalg}, 'l word, 'b) function_contract\<close> where
  [crush_contracts]: \<open>abs_diff_contract x y \<equiv>
    let pre = \<langle>True\<rangle> in
    let post = \<lambda>ret. \<langle>ret = (if x < y then y - x else x - y)\<rangle> in
      make_function_contract pre post\<close>
ucincl_auto abs_diff_contract

lemma abs_diff_spec [crush_specs]:
  shows \<open>\<Gamma> ; abs_diff x y \<Turnstile>\<^sub>F abs_diff_contract x y\<close>
by (crush_boot f: abs_diff_def contract: abs_diff_contract_def) (crush_base simp add: abs_diff_core_def)
...
definition checked_add_contract :: \<open>'a::{len} word \<Rightarrow> 'a word \<Rightarrow>
      ('s::{sepalg}, 'a word option, 'b) function_contract\<close> where
  [crush_contracts]: \<open>checked_add_contract w v \<equiv>
    let pre = \<langle>True\<rangle> in
    let post = \<lambda>ret. \<langle>ret = checked_add_core w v\<rangle> in
      make_function_contract pre post\<close>
ucincl_auto checked_add_contract

lemma checked_add_spec [crush_specs]:
  shows \<open>\<Gamma> ; checked_add w v \<Turnstile>\<^sub>F checked_add_contract w v\<close>
by (crush_boot f: checked_add_def contract: checked_add_contract_def) (crush_base simp add: checked_add_core_def)

definition checked_mul_contract :: \<open>'a::{len} word \<Rightarrow> 'a word \<Rightarrow>
      ('s::{sepalg}, 'a word option, 'b) function_contract\<close> where
  [crush_contracts]: \<open>checked_mul_contract w v \<equiv>
    let pre = \<langle>True\<rangle> in
    let post = \<lambda>ret. \<langle>ret = checked_mul_core w v\<rangle> in
      make_function_contract pre post\<close>
ucincl_auto checked_mul_contract

lemma checked_mul_spec [crush_specs]:
  shows \<open>\<Gamma> ; checked_mul w v \<Turnstile>\<^sub>F checked_mul_contract w v\<close>
by (crush_boot f: checked_mul_def contract: checked_mul_contract_def) (crush_base simp add: checked_mul_core_def)

definition checked_sub_contract :: \<open>'a::{len} word \<Rightarrow> 'a word \<Rightarrow>
      ('s::{sepalg}, 'a word option, 'b) function_contract\<close> where
  [crush_contracts]: \<open>checked_sub_contract w v \<equiv>
    let pre = \<langle>True\<rangle> in
    let post = \<lambda>ret. \<langle>ret = checked_sub_core w v\<rangle> in
      make_function_contract pre post\<close>
ucincl_auto checked_sub_contract

lemma checked_sub_spec [crush_specs]:
  shows \<open>\<Gamma> ; checked_sub w v \<Turnstile>\<^sub>F checked_sub_contract w v\<close>
by (crush_boot f: checked_sub_def contract: checked_sub_contract_def) (crush_base simp add: checked_sub_core_def)

definition checked_div_contract :: \<open>'a::{len} word \<Rightarrow> 'a word \<Rightarrow>
      ('s::{sepalg}, 'a word option, 'b) function_contract\<close> where
  [crush_contracts]: \<open>checked_div_contract w v \<equiv>
    let pre =  \<langle>True\<rangle> in
    let post = \<lambda>ret.\<langle>ret = checked_div_core w v\<rangle> in
      make_function_contract pre post\<close>
ucincl_auto checked_div_contract

lemma checked_div_spec [crush_specs]:
  shows \<open>\<Gamma> ; checked_div w v \<Turnstile>\<^sub>F checked_div_contract w v\<close>
by (crush_boot f: checked_div_def contract: checked_div_contract_def) (crush_base simp add: checked_div_core_def)
